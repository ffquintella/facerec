...
public static class ImageExtensions
{
	...
	public static byte[] ToArray(this SixLabors.ImageSharp.Image imageIn)
	{
		...
		{
			var jpegEncode = new JpegEncoder();
			
			imageIn.Save(ms, jpegEncode);
			return ms.ToArray();
		}
	}
	...
	/// <returns>An byte array containing the JPG format Image</returns>
	public static byte[] ToArray(this SixLabors.ImageSharp.Image imageIn, ImageEncoder encoder)
	{
		using(MemoryStream ms = new MemoryStream())
		{
	...
	public static SixLabors.ImageSharp.Image ToImage(this byte[] byteArrayIn)
	{
		...
		{
			var returnImage = SixLabors.ImageSharp.Image.Load(ms);
			return returnImage;
		}
	}
